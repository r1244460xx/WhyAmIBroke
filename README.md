...
### 4. 🧮 範圍內消費明細與即時浮動加法器 (Interactive Sticky SUM Adder)
- **全量明細展示**：列出所選條件下**範圍內全部交易項目**（不再受限於前 10 筆），依交易日期新至舊排序。
- **流暢自然滾動**：表格依資料量自適應全展開，避免被困在小視窗內滾動。
- **右側常駐浮動加法器 (Sticky Floating Adder)**：
  - 使用者一路往下滑動時，計算器**自動平滑跟隨視窗置頂常駐**，方便邊瀏覽邊隨時勾選試算。
  - 即時統計「**已選 N 筆，加總：NT$ XXX**」。
  - 附帶 **「全部重選」** 一鍵清空勾選。
